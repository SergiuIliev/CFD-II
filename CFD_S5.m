%% Computational Fluid Dynamics 2016 | Coursework 2: Section 5
%NAME: Compare numerical and analytical solutions for the Riemann problem
%GOAL: For the linear advection equation, discretised using a the two-step Lax-Friedrichs stencil, appropriate boundary conditions should are applied, then the Riemann problem should be solved to observe the variation of velocity, density, pressure, Mach no. and Entropy/Cv across the computational domain. For the problem is solved for three different mesh sizes.
      %Subsequently the analytical solution is loaded from a .dat file and the distribution of flow parameters is plotted for all cases at the time instance of interest, in order to compare the performance of the numerical scheme
%Created by: SERGIU PETRE ILIEV. Date: 01.II.2015
...
N=[101,201,10001];    % Resolution of the mesh (only 3 mesh density cases will be analysed here)
...
CFL=1;              % Assign the value of the CFL number (1 is marginally stable, but if the scheme runs correctly it is the most efficient value that can be chosen and still maintain stability)
...
%% Initial Flow conditions - N.B. This could also be written as in the Shock_tube_analytic.m case, where the program asks for pressure and density ratios - the methods are equivalent
...
p_fin= zeros(size(N,2),max(N));     % Initialise pressure array
rho_fin = zeros(size(N,2),max(N));  % Initialise density array
u_fin = zeros(size(N,2),max(N));    % Initialise velocity array
M_fin= zeros(size(N,2),max(N));     % Initialise Mach number array
s_fin= zeros(size(N,2),max(N));     % Initialise entropy array
x_fin=zeros(size(N,2),max(N));      % Initialise mesh spacing array
...
for i=1:1:size(N,2)
    ...
    NN=2*N(i);          % Since the Lax-Friedrichs scheme is going to be applied, the algorithm requires an efficient way of understanding the half-points i.e. i+1/2 and i-1/2. This can be done switching to a mesh double the size and thinking of each odd point as being a half-point
    ...
    % Initialize the gas parameters for all points in the mesh, based on the boundary conditions to the RHS and LHS of the diaphragm
    ...
    while t<time                        % Begin a while loop iterate the solver for each time step, until the time of interest is reached. N.B. Since t=0 initially, when t<time is no longer true, the last iteration computed was at t=time. 
        ...
        % Check if the position tracker is at a half-point or not and apply the appropriate solver to compute U
        if mod(l,1) %if the position tracker is even
        ...
        % Find all flow parameters at this timestep across the mesh and store them in their designated arrays
        rho = U(1,1:NN);
        u   = U(2,1:NN)./rho;
        E   = U(3,1:NN)./rho;
    ...
    % Having solved the problem for this specific mesh size, determine and store the flow parameters required for plotting at the time instance of interest i.e. the last time instance where t=time
...
%% Evaluating the performance of the scheme (Section 5)
% Load the data obtained from the analytical solver 
load CFD_S5_analytical.dat
rho_a = CFD_S5_analytical(:,2);
u_a   = CFD_S5_analytical(:,4);
p_a   = CFD_S5_analytical(:,3);
M_a   = CFD_S5_analytical(:,5);
s_a   = CFD_S5_analytical(:,6);
x_a   = CFD_S5_analytical(:,1);

%% Plot the results for the flow parameters of interest and the time of interest (this section needs to be changed if using a different number of mesh points, by using a simple: for k=1:1:size(N,2) {make the figures using the counter i.e. replacing the three entries for each one with one that incorporates the counter} end, as only the first three will be displayed here - this is because formatting work had to be done in order to differentiate between lines in the report, should it be printed B&W)
...
plot(x_a,u_a, '.-', 'LineWidth',0.5);                               % Plot the analytical solution
xlabel('x');                                                        % Label the x axis
ylabel('Velocity');                                                 % Label the y axis
legend('N=101', 'N=201', 'N=10001', 'Exact', 'Location','northeast')  % Specify the legend and its position
...
plot(x_a,p_a, '.-', 'LineWidth',0.5);                               % Plot the analytical solution
xlabel('x');                                                        % Label the x axis
ylabel('Pressure');                                                 % Label the y axis
legend('N=101', 'N=201', 'N=301', 'Exact', 'Location','northeast')  % Specify the legend and its position
...
plot(x_a,rho_a, '.-', 'LineWidth',0.5);                             % Plot the analytical solution
xlabel('x');                                                        % Label the x axis
...
plot(x_a,M_a, '.-', 'LineWidth',0.5);                               % Plot the analytical solution
xlabel('x');                                                        % Label the x axis
...
plot(x_a,s_a, '.-', 'LineWidth',0.5);                               % Plot the analytical solution
xlabel('x');                                                        % Label the x axis
